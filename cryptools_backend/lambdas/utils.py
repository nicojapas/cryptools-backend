...
def create_response(
    ...
    **kwargs
    ...
    """
    if body is None:
        response_body = {"message": message, "timestamp": datetime.utcnow().isoformat()}
...
def create_success_response(
    ...
    return create_response(status_code=200, message=message, data=data, **kwargs)


def create_error_response(
    error: str, status_code: int = 500, message: str = "Internal server error"
) -> Dict[str, Any]:
    """Create an error response."""
    return create_response(status_code=status_code, message=message, error=error)
...
def handle_api_errors(func):
    ...
    def wrapper(event, context):
        try:
            ...
            return create_error_response(error=str(e), message="Internal server error")
...
def handle_http_errors(func):
    ...
    def wrapper(event, context):
        try:
            ...
            return create_error_response(error=str(e), message=error_message)
...
def format_coin_data(coin: Dict[str, Any]) -> Dict[str, Any]:
    ...
    return {
        ...
        "marketCapChangePercentage24H": str(
            coin.get("market_cap_change_percentage_24h", 0)
        ),
